...
func TestBuildAuthorizationURL_UsesBuiltinSecretFallback(t *testing.T) {
	// SECURITY: Built-in client secret is no longer hardcoded.
	// The test must explicitly set the environment variable to simulate a configured secret.
	t.Setenv(GeminiCLIOAuthClientSecretEnv, "builtin-test-secret")
...
func TestEffectiveOAuthConfig_NoEnvSecret(t *testing.T) {
	// SECURITY: With the hardcoded secret removed, an empty env variable should now fail.
	t.Setenv(GeminiCLIOAuthClientSecretEnv, "")

	_, err := EffectiveOAuthConfig(OAuthConfig{}, "code_assist")
	if err == nil {
		t.Fatal("空环境变量且未配置自定义 OAuth 时应返回错误")
	}
}
